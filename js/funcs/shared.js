...
const getCourseDetails = async () => {
    ...
    if (courseDetail.comments.length) {
...
const createNewNewsLetter = async () => {
    ...
    if (res.ok) {
        showSwal("شما با موفقیت در خبرنامه سبزلرن عضو شدید", "success", "متوجه شدم", (result) => {
            location.href = "index.html"
        });
    }
}
...
const globalSearch = async () => {
    ...
    if (data.allResultCourses.length) {
        ...
        coursesSearchResultWrapper.insertAdjacentHTML('beforeend', `
            <div class="alert alert-danger">هیچ دوره ای برای جستجوی شما وحود ندارد</div>
        `)
    }
